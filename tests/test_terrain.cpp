...
// MAIN
int main(int argc, char **argv) { testPerlin(argc, argv); }
...
void testRepeatable(int argc, char **argv) {
    ...
    Mat<double> repeatable = perlin.generatePerlinNoise2D(
        size, {octaves, persistence, true, 0, freq, 0, 0});
...
void testPerlin(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            ...
            std::cout << "erreur dans la ligne de commande, utilisation des "
...
void testSubdivisions(int argc, char **argv) {
    ...
    TerrainSubdivisionTree *terrainTree =
        subdiv.generateSubdivisions(terrain, 4, 1);
