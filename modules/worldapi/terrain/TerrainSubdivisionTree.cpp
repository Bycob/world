...
TerrainSubdivisionTree::TerrainSubdivisionTree(
    const TerrainSubdivisionTree &terrain)
        : _terrain(std::make_unique<Terrain>(*terrain._terrain)),
          _subdivisionStage(terrain._subdivisionStage),
          _noisePart(terrain._noisePart), _x(terrain._x), _y(terrain._y),
          _parent(nullptr), _subdivideFactor(terrain._subdivideFactor) {
...
double TerrainSubdivisionTree::getZInterpolated(double x, double y,
                                                int stage) const {
    const TerrainSubdivisionTree &subtree = getSubtreeAt(x, y, stage);
...
int TerrainSubdivisionTree::getLevelDiff(
    const TerrainSubdivisionTree &terrain) const {
...
void TerrainSubdivisionTree::subdivide(int subdivideFactor) {
    ...
    _subtrees = std::vector<std::unique_ptr<TerrainSubdivisionTree>>(
        subdivideFactor * subdivideFactor);

    for (int x = 0; x < subdivideFactor; x++) {
        for (int y = 0; y < subdivideFactor; y++) {
            std::unique_ptr<TerrainSubdivisionTree> subtree =
                std::unique_ptr<TerrainSubdivisionTree>(
                    new TerrainSubdivisionTree(
                        new Terrain(_terrain->getResolution())));
...
const TerrainSubdivisionTree &TerrainSubdivisionTree::getSubtreeAt(
    double x, double y, int stage) const {
...
TerrainSubdivisionTree &TerrainSubdivisionTree::getSubtree(int xindex,
                                                           int yindex) const {
    if (xindex >= _subdivideFactor || yindex >= _subdivideFactor ||
        xindex < 0 || yindex < 0) {
...
Mesh *TerrainSubdivisionTree::convertToSubmesh(float rootSizeX, float rootSizeY,
                                               float rootSizeZ) const {
    ...
    // Calcul de la taille de ce terrain à partir de la taille du parent, ainsi
    // que de l'offset à appliquer.
    ...
    return _terrain->createMesh(offsetX - rootSizeX / 2,
                                offsetY - rootSizeY / 2, offsetZ, sizeX, sizeY,
                                sizeZ);
}

void TerrainSubdivisionTree::getLocalPosition(
    const TerrainSubdivisionTree &child, double globalPosX, double globalPosY,
    ...
    if (interTerrain == this) {
        ...
        while (interTerrain != this) {
            ...
            offsetX = ((double)interTerrain->_x + offsetX) /
                      interTerrain->_parent->_subdivideFactor;
            offsetY = ((double)interTerrain->_y + offsetY) /
                      interTerrain->_parent->_subdivideFactor;
