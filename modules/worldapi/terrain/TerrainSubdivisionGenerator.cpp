...
namespace world {
    ...
    TerrainSubdivisionGenerator::generateSubdivisions(Terrain &terrain, int subdivideFactor, int subdivisionsCount) {
        auto *result = new TerrainSubdivisionTree(terrain);
        generateSubdivisionLevels(*result, subdivideFactor, subdivisionsCount);
        return result;
    }

    void TerrainSubdivisionGenerator::generateSubdivisionLevels(TerrainSubdivisionTree &tree, int subdivideFactor,
    ...
    void TerrainSubdivisionGenerator::generateSubdivisionLevel(TerrainSubdivisionTree &terrain, int subdivideFactor) {
    ...
    void TerrainSubdivisionGenerator::generateSubdivision(TerrainSubdivisionTree &terrain, int xsub, int ysub) {
        //TODO Faire plusieurs tests pour voir si on obtient un meilleur résultat en changeant les paramètres de l'algo de perlin.
        /*mat &mat = createInBuf(xsub, ysub, terrain.getSubtree(xsub, ysub)._terrain->_array);
    ...
    void TerrainSubdivisionGenerator::adaptFromBuffer(TerrainSubdivisionTree &tree, int xsub, int ysub) const {
