...
namespace world {
    ...
    HeightMapInputStream::HeightMapInputStream(const world::Terrain &terrain, double offset, double scale,
                                               world::HeightMapFormat format)
            : _offset(offset), _scale(scale), _format(format), _terrain(terrain) {

    }

    HeightMapInputStream::HeightMapInputStream(const world::HeightMapInputStream &stream)
            : _terrain(stream._terrain) {

    }

    void HeightMapInputStream::setOffset(double offset) {
        _offset = offset;
    }

    void HeightMapInputStream::setScale(double scale) {
        _scale = scale;
    }
