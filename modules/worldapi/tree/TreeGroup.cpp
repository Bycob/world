...
void TreeGroup::collect(ICollector &collector,
	...
	// we choose the render mode of the tree group : 0:none / 1:together / 2:per tree
	int renderMode = 0;
...
void TreeGroup::regenerateGroup() {
	for (vec3d &pos : _treesPositions) {
		// trunk
		// TODO utiliser le générateur d'arbres pour générer une version low poly du tronc avec peu de branches.
		...
		for (int i = 0; i < 3; ++i) {
			...
			int ids[][3] = { { startIndex + 2 * i, startIndex + (2 * i + 2) % 6, startIndex + 2 * i + 1},
				{ startIndex + (2 * i + 2) % 6, startIndex + (2 * i + 2) % 6 + 1, startIndex + 2 * i + 1 } };
		...
		// leaves
		// TODO utiliser l'algorithme "QuickHull" pour générer des enveloppes convexes
		...
		for (int i = 0; i < ringCount; ++i) {
			for (int j = 0; j < segmentCount; ++j) {
				...
				vec3d vert = leavesCenter + vec3d{ cos(angle) * radius[i], sin(angle) * radius[i], height[i] };
				_leavesMesh.newVertex(vert);
				
				if (i != ringCount - 1) {
					int ringOffset = segmentCount * i;
					int totalOffset = startIndex + ringOffset;
					int ids[][3] = { { totalOffset + j, totalOffset + (j + 1) % 7, totalOffset + segmentCount + j },
						{ totalOffset + (j + 1) % 7, totalOffset + segmentCount + (j + 1) % 7, totalOffset + segmentCount + j } };
