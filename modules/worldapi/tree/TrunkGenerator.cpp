...
TrunkGenerator::TrunkGenerator(int segmentCount, double faceSize,
                               double mergeSize, int mergeRes)
        : _segmentCount(segmentCount), _faceSize(faceSize),
          _mergeSize(mergeSize), _mergeRes(mergeRes) {}
...
void TrunkGenerator::fillBezier(Mesh &trunkMesh, const BezierCurve &curve,
                                int divCount, double startWeight,
                                double endWeight, int mergePos) const {
    ...
    for (int i = 1; i <= divCount; i++) {
        ...
        const double localRadius =
            (sqrt(endWeight) * t + sqrt(startWeight) * (1 - t)) * 0.15;
        ...
        double offAngle =
            getAngle(oldAx, ax) * (abs(d) < 0.0001 ? 0 : d / abs(d));
        int offset = (int)(offAngle / (2 * M_PI) * _segmentCount + 0.5);
        // std::cout << getAngle(oldAx, ax) << " " << offAngle << " " << offset
        // << " " << oldAx << " " << ax << std::endl;
        ...
        for (int j = 0; j < _segmentCount; j++) {
            ...
            vec3d pt = ax * (cos(angle) * localRadius) +
                       ay * (sin(angle) * localRadius);
            ...
            int j1 = mod(j + offset, _segmentCount);
...
void TrunkGenerator::populateTrunkMesh(Mesh &trunkMesh,
                                       const Node<TreeInfo> *node,
                                       double mergeLenParent,
                                       double mergeWeight) const {

    auto getEndWeight =
        [](const std::vector<Node<TreeInfo> *> &children) -> double {
        if (children.size() == 0)
            return 0;
    ...
    /* On génére la branche, puis les joints avec les branches suivantes, ou le
bout de la branche s'il n'y a pas de branches suivantes. */
    ...
    const vec3d parentPos =
        parent == nullptr ? node->getPosition() : parent->getPosition();
    ...
    if (length > 0 || parent == nullptr) {
        ...
        if (parent != nullptr) {
            ...
            vec3d pt2 =
                parentPos + unitVector * (info._size - mergeLenChild -
                                          sqrt(avgChildrenWeight) *
                                              0.15); // avgWeight = facteur pour
            ...
            const double w2 =
                avgChildrenWeight + (mergeLenChild / info._size) *
                                        (node->getWeight() - avgChildrenWeight);
            mergeWeight1 = w2;

            fillBezier(trunkMesh, curve, divider, w1, w2,
                       trunkMesh.getVerticesCount() - _segmentCount);
        } else {
    ...
    // On retient la position de la dernière corolle du parent pour lier tous
    // les enfants.
    const int mark = trunkMesh.getVerticesCount() - _segmentCount;
    // Origine commune côté parent
    const vec3d parentSide =
        pos - (pos - parentPos) * (mergeLenChild / info._size);
    const vec3d parentDirection =
        (pos - parentPos) * (0.5 * mergeLenChild / info._size);
    ...
    for (const Node<TreeInfo> *child : children) {
        ...
        const double newMergeLen = min(childInfo._size / 2, (double)_mergeSize);
        const double avgChildChildrenWeight =
            getEndWeight(child->getChildrenOrNeighboursAccess());

        // Création de la courbe de Bezier
        const vec3d childSide =
            pos + (childPos - pos) * (newMergeLen / childInfo._size);
        const vec3d childDirection =
            (pos - childPos) * (0.5 * newMergeLen / childInfo._size);

        BezierCurve curve(parentSide, childSide, parentDirection,
                          childDirection);

        // Calcul des poids
        const double w1 = mergeWeight1;
        const double w2 = child->getWeight() -
                          (newMergeLen / childInfo._size) *
                              (child->getWeight() - avgChildChildrenWeight);
