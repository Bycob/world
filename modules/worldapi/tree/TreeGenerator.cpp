	...
	void TreeGenerator::populateTrunkMesh(Mesh &trunkMesh, const Node<TreeInfo> *node,
		...
		if (length > 0 || parent == nullptr) {
			...
			if (parent != nullptr) {
				...
				vec3d pt2 = parentPos + unitVector * (info._size - mergeLenChild - sqrt(avgChildrenWeight) *
																				   0.15); // avgWeight = facteur pour réduire l'encombrement au niveau du merge
		...
		for (const Node<TreeInfo> *child : children) {
			...
			const double w2 = child->getWeight() -
							  (newMergeLen / childInfo._size) * (child->getWeight() - avgChildChildrenWeight);
