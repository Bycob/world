...
void TreeSkelettonGenerator::process(Tree &tree) {
    ...
    primaryNode->setPosition(
        0, 0, _seedLocation(primaryNode->getInfo(), 0, 0, 0, 0, 0));
...
void TreeSkelettonGenerator::forkNode(Node<TreeInfo> *node) {
    ...
    // std::cout << info._phi << " " << info._theta << " " << info._level << " "
    // << info._size << std::endl;
    ...
    if (info._level != 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            const double newTheta = (2.0 * M_PI * i) / count +
                                    _offsetTheta(info, count, 0, 0, 0, 0);
            ...
            const double newSize =
                info._size * _sizeFactor(info, count, newTheta, phi, weight, 0);

            // Ajout du noeud
            Node<TreeInfo> *newNode = node->createChild(
                weight, pos.x + cos(newTheta) * sin(newPhi) * newSize,
    ...
    for (auto addedNode : addedNodes) {
        ...
        newInfo._size =
            vec3d::length(node->getPosition(), addedNode->getPosition());
