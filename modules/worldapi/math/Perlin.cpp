...
void Perlin::join(arma::Mat<double> &mat1, arma::Mat<double> &mat2,
                  const Direction &direction, int octaves, double frequency,
                  double persistence, bool joinableSides) {
    ...
    for (int i = 1; i <= octaves; i++) {
        ...
        for (int y = joinDepth - period - 1; y >= period; y -= period) {
            for (int x = 0; x < length; x += period) {
                ...
                // Permet à ce que la dernière période s'étende sur toute la
                // place disponible
                if (x >= length - period) {
                    x = length - 1;
                }
                ...
                if (joinableSides && (x == length - 1 || x == 0)) {
                    octave(x, y) = Interpolation::interpolateCosine(
                        0, at(x, 0), joinDepth - 1, at(x, joinDepth - 1), y);
                    // if (x == length - 1) std::cout << "y=" << y << " : " <<
                    // octave(x, y) << std::endl;
                } else {
        ...
        for (int x = 0; x < length; x++) {
            for (int y = 0; y < joinDepth; y++) {
                ...
                if (y != 0 && y != joinDepth - 1 &&
                    (x % period != 0 || minusY % period != 0 || y < period ||
                     minusY <= period)) {
                    ...
                    // Interpolation en x
                    const std::function<double(int)> interpolateX =
                        [&](int borneY) -> double {
                        if (borneY == 0 || borneY == joinDepth - 1) {
                            ...
                            // << "at " << borneX2 << " there is " <<
                            // octave(borneX2, borneY) << std::endl;
                            return Interpolation::interpolateCosine(
                                borneX1, octave(borneX1, borneY), borneX2,
