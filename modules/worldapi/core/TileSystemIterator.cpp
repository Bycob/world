...
TileSystemIterator::TileSystemIterator(const TileSystem &tileSystem, 
	const IResolutionModel &resolutionModel, const WorldZone &zone)
...
TileCoordinates TileSystemIterator::operator*() {
	return _current;
}

// TODO iterator end is poorly managed
bool TileSystemIterator::endReached() const {
	return _endReached;
}
...
bool TileSystemIterator::isTileRequired(TileCoordinates coordinates) {
	...
	double resolutionInTile = _resolutionModel.getMaxResolutionIn({ lower, upper });
