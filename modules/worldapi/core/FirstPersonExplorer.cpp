...
namespace world {
    ...
    void FirstPersonExplorer::setFOV(double fov) {
        _fov = fov;
    }
    ...
    vec3d FirstPersonExplorer::getChunkNearestPoint(const WorldZone &zone) {
        ...
        return {
            clamp(_position.x, lower.x, upper.x),
    ...
    double FirstPersonExplorer::getResolutionAt(const vec3d &pos) {
        ...
        return _eyeResolution / (_fov * M_PI / 180 * length); // _fov * length can be seen as the "image size"
    }
}
