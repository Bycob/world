...
Collector::Collector() : _internal(new PCollector()) {}

Collector::~Collector() { delete _internal; }
...
void Collector::removeItem(const ItemKey &key) { _internal->_items.erase(key); }
...
void Collector::addMaterial(const ItemKey &key, const Material &material) {
    ...
    _internal->_items.at(key)->_internal->_materials.emplace(material.getName(),
                                                             material);
}

void Collector::addTexture(const ItemKey &key, const std::string &texName,
                           const Image &texture) {
    if (_internal->_disabled.find(key) != _internal->_disabled.end()) {
        return;
    }
...
CollectorIterator Collector::iterateItems() { return CollectorIterator(*this); }
...
CollectorItem::CollectorItem(const ICollector::ItemKey &key,
                             const Object3D &object3D)
        : _internal(new PCollectorItem(key, object3D)) {}

CollectorItem::~CollectorItem() { delete _internal; }

const Object3D &CollectorItem::getObject3D() const {
    return _internal->_object3D;
}

Object3D &CollectorItem::getObject3D() { return _internal->_object3D; }

optional<const Material &> CollectorItem::getMaterial(
...
optional<CollectorItem::texture> CollectorItem::getTexture(
    const std::string &key) const {
    auto it = _internal->_image.find(key);

    if (it != _internal->_image.end()) {
        std::string uniqueStr =
            ICollector::ItemKeys::toString(_internal->_myKey) + "/" + key;
...
CollectorIterator::~CollectorIterator() { delete _internal; }

void CollectorIterator::operator++() { _internal->_objectIt++; }
