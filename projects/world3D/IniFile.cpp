...
IniFile IniFile::read(const std::string &filename) { return IniFile(""); }

IniFile::IniFile() {}

IniFile::IniFile(const std::string &content) { parse(content); }

bool IniFile::hasKey(const std::string &key) const {
    return _values.find(key) != _values.end();
}

int IniFile::getKeyCount() const { return (int)_values.size(); }

std::string IniFile::getValue(const std::string &key) const {
    return _values.at(key);
}

std::string IniFile::getOrSetValue(const std::string &key,
