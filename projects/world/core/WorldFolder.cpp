...
WorldFolder::WorldFolder(const std::string &path) : _path(path), _filelist() {
...
bool WorldFolder::exists() const { return _exists; }
...
bool WorldFolder::hasFile(const std::string &name) const {
    return std::find(_filelist.begin(), _filelist.end(), name) !=
           _filelist.end();
}

void WorldFolder::refresh() { init(); }
