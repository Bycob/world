...
WORLD_REGISTER_CHILD_CLASS(ITextureProvider, GroundTextureGenerator,
...
Image &GroundTextureGenerator::getTexture(int layer, int lod) {
    ...
    if (_cache.hasImage(imageId)) {
        auto in = _images.insert({{layer, lod}, _cache.readImage(imageId)});
    ...
    if (layer >= _colors.size())
        throw std::runtime_error(
            "Layer index " + std::to_string(layer) +
            " is out of range (max = " + std::to_string(_colors.size()) + ")");

    auto in = _images.insert(
        {{layer, lod}, Image(_texWidth, _texWidth, ImageType::RGBA)});
