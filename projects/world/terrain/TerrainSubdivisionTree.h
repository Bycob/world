 ...
 * niveaux de détail plus élevés. On peut ensuite choisir le niveau de détail
 * qu'on veut observer.*/
...
public:
    ...
    int getSubdivisionStage() const { return _subdivisionStage; }
    ...
    int getSubdivideFactor() const { return _subdivideFactor; }
     ...
     *@param stage Le niveau du terrain dans la hierarchie. Va de 0, qui
     *représente ce terrain, à n, avec n le nombre d'étage de subdivisions
     *réalisé depuis ce terrain.
    ...
    const TerrainSubdivisionTree &getSubtreeAt(double x, double y,
                                               int stage = 1) const;

    TerrainSubdivisionTree &getSubtree(int xindex, int yindex) const;

    /** Convertit ce terrain en submesh, en indiquant la taille du terrain
     *parent au niveau de détail le plus bas, c'est à dire le parent le plus
     *éloigné de ce terrain. Le mesh issu de la conversion représentera donc une
     *fraction de ce parent éloigné. Si le niveau de subdivision de ce terrain
     *est 0, alors cette méthode produit le même résultat que la méthode
     *#convertToMesh(float, float, float). */
    Mesh *convertToSubmesh(float rootSizeX = 1, float rootSizeY = 1,
                           float rootSizeZ = 0.4) const;
...
private:
    ...
    /** Avec child le terrain passé en paramètres, enfant du nième degré, et
     *root le terrain parent le plus haut dans sa hierarchie, on peut obtenir la
     *position sur child à partir de la position sur root passée en paramètres.
     *Les coordonnées s'exprime en pourcentage des dimensions des terrains.*/
    void getLocalPosition(const TerrainSubdivisionTree &child,
                          double globalPosX, double globalPosY,
