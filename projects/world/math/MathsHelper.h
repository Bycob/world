...
template <typename T> class WORLDAPI_EXPORT Function {
public:
    virtual T operator()(T x) const = 0;
};

template <typename T> constexpr inline auto abs(T v) -> T {
    return v >= 0 ? v : -v;
}
...
constexpr inline auto clamp(T1 value, T2 min, T3 max)
    -> decltype(value + min + max) {
